...
入站：OpenAI 请求体 → Anthropic messages 体（system/developer 提取、content 分块、
      tool_calls / tool_result / 图片 data URL best-effort 映射）。
出站：Anthropic 响应（JSON 或 SSE 事件流）→ OpenAI 格式。

原则：畸形输入保持宽容，映射不了的部件安静跳过，绝不放大请求失败。
"""
...
def _image_block(url: str) -> dict | None:
    """data:image/...;base64,xxx → Anthropic image block；外链 URL 跳过。"""
...
def iter_sse_events(lines) -> list[dict]:
    """Anthropic SSE 文本行 → 事件 dict 列表（宽松解析，坏行跳过）。"""
    events: list[dict] = []
    ...
    for raw in lines:
        ...
        line = raw.rstrip("\r\n")
        if not line:
            if current_type and data_lines:
                try:
                    data = json.loads("".join(data_lines))
                    if isinstance(data, dict):
                        data.setdefault("type", current_type)
                        events.append(data)
                except ValueError:
                    pass
            current_type = None
