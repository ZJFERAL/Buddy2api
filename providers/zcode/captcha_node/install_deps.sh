#!/bin/bash
# 安装验证码求解器依赖（happy-dom + undici，Zcode2Api3 design port）
...
if command -v npm &> /dev/null; then
    ...
    npm install --no-progress happy-dom@^20.14.0 undici@^8.10.2
else
    ...
    try {
        execSync('npm install --no-progress happy-dom@^20.14.0 undici@^8.10.2', {
            stdio: 'inherit',
...
# 验证安装
if [ -d "node_modules/happy-dom" ] && [ -d "node_modules/undici" ]; then
    echo "✅ All dependencies installed successfully!"
    ls -la node_modules/ | grep -E "(happy-dom|undici)"
else
    echo "⚠️ Some dependencies missing:"
    test -d "node_modules/happy-dom" && echo "  ✓ happy-dom" || echo "  ✗ happy-dom"
    test -d "node_modules/undici" && echo "  ✓ undici" || echo "  ✗ undici"
fi