...
console.log(`🎯 Solving captcha: Scene=${SCENE}, Region=${REGION}, Prefix=${PREFIX}`);
...
function waitFor(cond, t = 15000) {
  return new Promise((res, rej) => {
    const s = Date.now();
    const i = setInterval(() => { let ok=false; try{ok=cond();}catch{} if(ok){clearInterval(i);res();} else if(Date.now()-s>t){clearInterval(i);rej(new Error('timeout'));} }, 100);
  });
}
...
(async () => {
  try {
    ...
    window.initAliyunCaptcha({
      ...
      getInstance: (inst) => { try { (inst.startTracelessVerification || inst.show).call(inst); } catch (e) { console.error('Failed to start:', e.message); } },
      success: (param) => { 
        console.log('✅ Verification successful!'); 
        console.log('VERIFY_PARAM=' + param); 
        process.exit(0); 
      },
      fail: () => { console.error('❌ Verification failed'); process.exit(4); },
      onError: () => { console.error('❌ Verification error'); process.exit(5); }
    });
    
    setTimeout(() => { 
      console.error('⏱️ Timeout after 25 seconds');
      process.exit(2); 
    }, 25000);
